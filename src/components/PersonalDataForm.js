...
import BMIpersonalPopUp from "./BMIpersonalPopUp";
...
export default function PersonalDataForm() {
  ...
  const getBmiDetails = (bmiValue) => {
    ...
    if (Number.isNaN(bmiNumber)) {
      return {
        category: "ไม่สามารถคำนวณได้",
        advice:
          "กรุณาลองใหม่อีกครั้งหรือติดต่อเจ้าหน้าที่เพื่อรับความช่วยเหลือ",
      };
    }
    ...
    if (bmiNumber < 18.5) {
      return {
        ...
        advice:
          "พิจารณาเพิ่มปริมาณอาหารที่มีคุณภาพและปรึกษานักโภชนาการหากจำเป็น",
      };
    }
    ...
    if (bmiNumber < 23) {
      return {
        ...
        advice:
          "รักษาพฤติกรรมการรับประทานอาหารและการออกกำลังกายที่ดีอย่างสม่ำเสมอ",
      };
    }
    ...
    if (bmiNumber < 25) {
      return {
        ...
        advice:
          "เพิ่มกิจกรรมทางกายเล็กน้อยและเลือกอาหารที่มีไขมันต่ำเพื่อลดน้ำหนัก",
      };
    }
    ...
    if (bmiNumber < 30) {
      return {
        ...
        advice:
          "ควรวางแผนการออกกำลังกายและปรึกษาผู้เชี่ยวชาญเพื่อควบคุมน้ำหนัก",
      };
    }

    return {
      category: "โรคอ้วนระดับ 2",
      advice: "ควรปรึกษาแพทย์เพื่อวางแผนการลดน้ำหนักอย่างเหมาะสมและปลอดภัย",
    };
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      const { category, advice } = getBmiDetails(bmi);

      console.log("Form submitted:", { ...formData, bmi, category });
      setBmiData({ value: bmi, category, advice });
      ...
      <div className="max-w-2xl mx-auto p-6 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 rounded-2xl shadow-xl border border-gray-100">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            กรุณากรอกข้อมูลเพื่อรับคำแนะนำที่เหมาะสมกับคุณ
          </p>
        </div>
        ...
        <BMIpersonalPopUp
          ...
          advice={bmiData?.advice}
          onClose={handleCloseBmiModal}
        />
      </div>
      <div className="max-w-2xl mx-auto mt-6 flex justify-end">
        <button onClick={AibuttonClick} className="rounded-xl border border-purple-300 px-6 py-3 text-sm font-semibold text-purple-600 shadow-sm transition hover:-translate-y-0.5 hover:border-purple-400 hover:text-purple-700 hover:shadow-md">
          วิเคราะห์โรคด้วย AI
        </button>
      </div>
    </>
